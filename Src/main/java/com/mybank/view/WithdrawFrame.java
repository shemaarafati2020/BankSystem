...
public class WithdrawFrame extends JFrame {
    ...
    public WithdrawFrame(User user, DashboardFrame parent) {
        this.user = user;
        this.parent = parent;
        setTitle("Withdraw Funds - " + (user.getFullName() == null ? user.getUsername() : user.getFullName()));
        setSize(460, 320);
    ...
    private void init() {
        ...
        gbc.gridx = 0;
        gbc.gridy++;
        form.add(new JLabel("Purpose:"), gbc);
        gbc.gridx = 1;
        ...
        balanceLabel.setForeground(new Color(0x8B0000));

        JPanel footer = new JPanel(new BorderLayout());
        footer.add(balanceLabel, BorderLayout.WEST);

        JPanel buttons = new JPanel();
        JButton submit = new JButton("Process Withdrawal");
        ...
        submit.addActionListener(e -> processWithdrawal());
    }

    private void processWithdrawal() {
        String amtText = amountField.getText().trim();
        if (amtText.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Enter an amount to withdraw.");
            return;
        }
        ...
        String description = reason + (note.isEmpty() ? "" : " - " + note);
            ...
            try {
                ...
                try (PreparedStatement ps = c.prepareStatement("SELECT balance FROM users WHERE user_id = ? FOR UPDATE")) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        if (!rs.next()) throw new Exception("User account not found.");
                ...
                if (!new TransactionDAO().create(c, tx)) {
                    throw new SQLException("Failed to record withdrawal transaction");
                }

                c.commit();
                JOptionPane.showMessageDialog(this, "Withdrawal recorded successfully.");
                parent.refreshBalances();
                dispose();
            } catch (Exception ex) {
                c.rollback();
                JOptionPane.showMessageDialog(this, "Failed to process withdrawal: " + ex.getMessage());
            } finally {
